    ...
    >
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 lg:px-10">
        <h2 className="title font-semibold">
          Sou Raphael Elias, <br />
          desenvolvedor web e estudante, <br />
          vivendo e trabalhando <br /> no Espírito Santo.
        </h2>
        <p className="max-w-sm text-sm text-neutral-700 md:text-base dark:text-neutral-300">
          Nos últimos 2 anos, venho atuando como freelancer, construindo
          soluções web modernas e funcionais. Abaixo, você pode acompanhar a
          linha do tempo da minha trajetória até aqui.
        </p>
        <div className="flex flex-row gap-3">
          {socialLinks.map((link, index) => (
            <Link className="py-3 font-bold" href={link.href} key={index}>
              {link.title}
            </Link>
          ))}
        </div>
